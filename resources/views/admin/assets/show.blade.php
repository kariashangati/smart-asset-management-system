    ...
    <div class="card-body">
        <div class="row">
            <div class="col-md-4 text-center">
                @if($asset->image)<img src="{{ Storage::url($asset->image) }}" class="img-fluid rounded" style="max-height: 300px;">@else<p>No image available</p>@endif
            </div>
            <div class="col-md-8">
                <table class="table table-bordered">
                    ...
                    <tr><th>Status</th><td><span class="badge bg-{{ $asset->status == 'active' ? 'success' : ($asset->status == 'inactive' ? 'warning' : 'danger') }}">{{ ucfirst($asset->status) }}</span></td></tr>
