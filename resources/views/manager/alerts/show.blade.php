...
<div class="page-heading">
    <div>
        ...
        <p class="breadcrumb-trail">
            ...
            <a href="{{ route('manager.alerts.index') }}">Alerts</a> /
            <span>Detail</span>
        </p>
    </div>
</div>
...
<div class="content-card">
    ...
    <div class="button-row mt-4">
        @if($alert->status === 'unread')
            <form method="POST" action="{{ route('manager.alerts.mark-read', $alert) }}">
                @csrf @method('PATCH')
                <button type="submit" class="btn btn-primary">Mark as read</button>
            </form>
        @endif
        @if($alert->status !== 'resolved')
            <form method="POST" action="{{ route('manager.alerts.mark-resolved', $alert) }}">
                @csrf @method('PATCH')
                <button type="submit" class="btn btn-success">Resolve alert</button>
            </form>
        @endif
        <a href="{{ route('manager.alerts.index') }}" class="btn btn-outline">Back to list</a>
    </div>
    @endif
</div>
@endsection
